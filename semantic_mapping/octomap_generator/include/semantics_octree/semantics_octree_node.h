...
namespace octomap
{
  ...
  protected:
    ...
    point3d center; ///<Center of the node
  };
  ...
  std::ostream& SemanticsOcTreeNode<SEMANTICS>::writeData(std::ostream &s) const {
    ...
    //Color sem_color = Color(255, std::max(255 - int(semantics.getTimestamp() - semantics.start_time), 0), 255);
    Color sem_color = semantics.getSemanticColor();
    /*Color sem_color = Color(255, 255 - int(semantics.getConfidence() * 100 + 50), 255);
    if (semantics.getSemanticColor() == Color(255,255,255))
      sem_color = Color(255,255,255);*/
